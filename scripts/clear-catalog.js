 ...
 * - PostgreSQL: catalog_items (barcha qatorlar)
 * - Disk: uploads/catalog/ rasmlar
 *
...
async function main() {
  ...
  if (!confirmed) {
    console.error(
      'Tasdiq kerak (faqat catalog o\'chadi):\n' +
        '  npm run clear-catalog\n' +
        '  node scripts/clear-catalog.js --confirm',
    );
    process.exit(1);
  ...
  try {
    ...
    console.log(`catalog_items: ${beforeCount} -> ${afterCount}`);
    console.log(
      `uploads/catalog: ${files.skipped ? 'bo\'sh papka yaratildi' : `${files.removed} fayl o'chirildi`}`,
    );
    console.log('Tayyor.');
  } finally {
    await client.end();
